...
Sizes are at 390 px, then the clamp for larger screens.

| Token | Family and settings | Size / line height / tracking | Clamp | Used for |
|---|---|---|---|---|
| `type-masthead` | Imbue, opsz 100, wght 300 | 132 / 0.82 / −0.01em | `clamp(112px, 34vw, 240px)` | the couple's first names in the signature treatment; nothing else |
| `type-figure` | Imbue, opsz 100, wght 200 | 220 / 0.8 / −0.02em | `clamp(180px, 56vw, 360px)` | the Wedding's "15"; nothing else |
| `type-display` | Imbue, opsz 90, wght 300 | 72 / 0.9 / −0.005em | `clamp(60px, 18.5vw, 150px)` | event names, venue name |
| `type-title` | Imbue, opsz 60, wght 300 | 44 / 1.0 / 0 | `clamp(38px, 11.3vw, 84px)` | the welcome heading, story pull quotes, RSVP and closing titles |
| `type-line` | Imbue, opsz 32, wght 300 | 28 / 1.1 / +0.005em | `clamp(26px, 7.2vw, 44px)` | emotional lines, at most 12 words |
| `type-body` | Archivo, wdth 100, wght 400 | 15 / 1.55 / 0 | `clamp(15px, 3.85vw, 18px)` | story text, venue description; at most 45 words per block |
| `type-info` | Archivo, wdth 100, wght 420 | 16 / 1.4 / 0 | `clamp(16px, 4.1vw, 19px)` | times, venues, dates in sentences |
| `type-label` | Archivo, wdth 118, wght 520, UPPERCASE | 11 / 1.2 / +0.16em | `clamp(11px, 2.8vw, 13px)` | labels, surnames, links, captions headers, the Sample mark (10 px) |
| `type-caption` | Archivo, wdth 100, wght 400 | 13 / 1.4 / 0 | `clamp(13px, 3.3vw, 15px)` | photo captions |
| `type-mark` | Noto Serif Devanagari, wdth 75, wght 450 | 14 / 1 / 0 | `clamp(14px, 3.6vw, 17px)` | chapter numerals ०१–०८ |
| `type-input` | Archivo, wdth 100, wght 400 | 18 / 1.3 / 0 | fixed 18 px | RSVP inputs (18 px prevents iOS zoom on focus) |

**Scale logic.** The steps jump rather than creep (11 → 15 → 28 → 44 → 72 → 132). *Why:* editorial pages get
...
1. **Three sizes per viewport, maximum** (a label, a reading size, one display size).
...
12. **Minimum sizes.** 13 px for any reading text, 10 px for the Sample mark only, 11 px for labels.
...
| "MALHOTRA" | `type-label`, left 22 px, 14 px below Rohan's baseline | surnames whisper so first names can speak |
| "Anaya" | `type-masthead`, flush right at 372 px, baseline at 61% | low and right: the diagonal that the thread follows |
| "KAPOOR" | `type-label`, right-aligned at 368 px, 14 px below Anaya's baseline | |
...
- Body text 15 px minimum with 1.55 line height; nothing below 13 px except labels (11 px) and the Sample mark.
