/* Stages the groom's-family mirror of the Lal & Sona invitation (slug: diptanshu-weds-nidhi) as a flat
   site for Vercel: the design at the root, the shared engine beside it at engine/, and the ../../engine/
   references rewritten to match. Nothing is edited in site/ — this only ever writes into the staging
   folder. Usage: node tools/deploy-diptanshu-weds-nidhi.mjs */
import fs from 'node:fs/promises';
import path from 'node:path';

const root = path.resolve(import.meta.dirname, '..');
const design = path.join(root, 'site', 'invitations', 'diptanshu-weds-nidhi');
const engine = path.join(root, 'site', 'engine');
const shared = path.join(root, 'site', 'shared');
const out = path.join(root, 'vercel-diptanshu-weds-nidhi');
